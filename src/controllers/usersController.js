  ...
  try {
    ...
    let validate = User.findOne({
      where: {
        role: "admin",
        token:
          "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOjEsInVzZXJuYW1lIjoiTXVyaWdvbCIsImlhdCI6MTY1OTk3NzU2OSwiZXhwIjoxNjU5OTgxMTY5fQ.wNs3yHViFXK4FlJBXvun9H6oY2yDVd4J6KltQ-yu0zA",
      },
    });
    if (validate) {
      return res
        .status(200)
        .send({ type: "success", message: "Acesso válido!" });
    } else {
      return res
        .status(200)
        .send({ type: "error", message: "Acesso negado!" });
    }
  } catch (error) {
    return res.status(200).send({
